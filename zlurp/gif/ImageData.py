class ImageData:
    def __init__(self,stream,colors):
        ...
        self.MAX_TABLE_SIZE  =256    # Impossible code = flush
        self.colors=colors
    ...
    def read_block(self,block_size):
        ...
        while self.block_index < block_size:
            ...
            #print(self.lookup)
            code=self.read_code()
            print("Index: 0x{0:02x}, Code: {1:02x}, Max code: {2:02x} Lookup Index {3:02X} BI:{4},BS:{5}".format(self.code_index,code,self.MAX_CODE_SIZE,self.next_table_index,self.block_index,block_size))
    ...
    def add_code_list_to_lookup(self,code_list):
        ...
        if self.next_table_index>=len(self.lookup):
            # print("Stretching lookup table")
            # print (self.next_table_index-len(self.lookup)+1)
            self.lookup=self.lookup+ [self.NOT_A_CODE]*(self.next_table_index-len(self.lookup)+1)
    ...
    def init_lookup_table(self):
       ...
       # print("MAX_CODE_SIZE:{0:02x}".format(self.MAX_CODE_SIZE))
        ...
        self.lookup=[self.NOT_A_CODE]*(4096)
        for index in range(0,self.colors):
